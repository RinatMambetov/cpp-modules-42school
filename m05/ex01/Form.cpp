...
Form::Form(std::string const &name, int gradeToSign, int gradeToExecute) try
    : m_name(name),
      m_isSigned(defaultSignStatus),
...
Form::Form(Form const &orig)
    : m_name(orig.getName()),
      m_isSigned(orig.getSignedStatus()),
      m_gradeToSign(orig.getGradeToSign()),
      m_gradeToExecute(orig.getGradeToExecute()) {}
...
void Form::beSigned(Bureaucrat const &b) {
  if (b.getGrade() <= getGradeToSign()) {
    ...
    std::cout << "Bureaucrat " << b.getName() << " couldn’t sign form "
              << getName() << ", because the grade is too low!\n";
    throw Form::GradeTooLowException();
  }
}

std::ostream &operator<<(std::ostream &out, Form const &form) {
  out << "Form " << form.getName() << " get signed status "
      << form.getSignedStatus() << ", get grade to sign "
      << form.getGradeToSign() << ", get grade to execute "
